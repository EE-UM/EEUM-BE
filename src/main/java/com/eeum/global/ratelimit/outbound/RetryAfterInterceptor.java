package com.eeum.global.ratelimit.outbound;
...
public class RetryAfterInterceptor implements ClientHttpRequestInterceptor {
  ...
  private static final int MAX_RETRY_AFTER_SECONDS = 5;

  private final TokenBucketRateLimiter rateLimiter;
  private final int maxAttempts;

  public RetryAfterInterceptor(TokenBucketRateLimiter rateLimiter, int maxAttempts) {
  ...
  private long parseRetryAfterSeconds(ClientHttpResponse response) {
    ...
    try {
      return Math.min(MAX_RETRY_AFTER_SECONDS, Math.max(0, Long.parseLong(value.trim())));
